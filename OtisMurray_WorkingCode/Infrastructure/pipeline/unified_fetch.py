...
def run_unified_fetch(mode: str = "fast") -> dict:
    ...
    t0 = time.time()
    try:
        tv_tickers = os.environ.get("TRADINGVIEW_TICKERS", "")
        tv_result = fetch_tradingview_news(
            tickers=tv_tickers.split(",") if tv_tickers else None,
            max_tickers=int(os.environ.get("TRADINGVIEW_MAX_TICKERS", "25")),
        )
