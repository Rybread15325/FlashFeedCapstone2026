...
def extract_tickers(title):
    ...
    for match in re.findall(r"(?:NYSE|NASDAQ|Nasdaq|AMEX)\s*:\s*([A-Z][A-Z0-9.-]{0,5})", text):
        found.add(match.upper())
    for match in re.findall(r"\(([A-Z][A-Z0-9.-]{0,5})\)", text):
        found.add(match.upper())
    ...
    words = set(re.findall(r"\b[A-Z][A-Z0-9.-]{1,5}\b", text))
    found.update(ticker for ticker in words if ticker in TRACKED_TICKERS)
