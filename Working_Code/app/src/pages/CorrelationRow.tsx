...
export function CorrelationRow({ entry: e }: { entry: CorrelationEntry }) {
  ...
  const confidence = e.confidence ?? Math.min(1, sampleSize / 30)
  ...
  return (
    <tr className="border-b border-slate-700/30 hover:bg-card-hover transition-colors">
      <td className="px-3 py-2.5">
        ...
        {e.generated && <span className="ml-2 text-[9px] uppercase text-neutral border border-border rounded px-1 py-0.5">live</span>}
      </td>
      ...
      <td className={clsx('px-3 py-2.5 font-mono text-xs', (e.news_sentiment ?? 0) >= 0 ? 'text-emerald-400' : 'text-red-400')}>
        {e.news_sentiment == null ? '—' : `${e.news_sentiment >= 0 ? '+' : ''}${e.news_sentiment.toFixed(2)}`}
      </td>
      ...
      <td className="px-3 py-2.5 font-mono text-xs text-neutral">{sampleSize}</td>
      <td className="px-3 py-2.5 font-mono text-xs text-neutral">{(confidence * 100).toFixed(0)}%</td>
      <td className="px-3 py-2.5">
