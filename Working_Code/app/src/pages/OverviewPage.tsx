...
function sentimentLabel(value?: string | number | null) {
  if (typeof value === 'number') return value >= 0 ? `+${value.toFixed(2)}` : value.toFixed(2)
...
export function OverviewPage() {
  ...
  const { data: socialData } = useSWR('/api/social/rolling?window_minutes=1440&limit=30', fetcher, { refreshInterval: 30_000 })
  ...
  const trackedTickerCount = stats?.tracked_ticker_count ?? stats?.tracked_tickers?.length ?? 250
  ...
  const avgAlignment = correlations.length
    ? correlations.reduce((sum, entry) => sum + Math.abs(Number(entry.correlation || 0)), 0) / correlations.length
    : null
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-6 gap-3">
        ...
        <Metric label="Tracked Tickers" value={compact(trackedTickerCount)} tone="text-sky-300" />
        ...
        <Metric label="Avg Align" value={avgAlignment == null ? '--' : avgAlignment.toFixed(2)} tone="text-yellow-300" />
      </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-[minmax(0,1.45fr)_minmax(320px,0.9fr)_minmax(300px,0.8fr)] gap-4">
              ...
              return (
                <div key={`${post.platform}-${ticker}-${idx}`} className="px-3 py-3">
                  <div className="flex items-center gap-2 text-[11px] mb-1">
                    ...
                    {ticker && <span className="font-mono text-accent">${ticker}</span>}
        ...
        <section className="min-w-0 bg-surface border border-border rounded-lg overflow-hidden">
          <SectionTitle title="Ticker Tracker" meta={tickerMentions.length ? `top ${tickerMentions.length}` : 'waiting'} />
          ...
          <SectionTitle title="Alignment" meta={`${correlations.length} signals`} />
          <div className="p-3 space-y-2">
  ...
  return (
    <a href={article.url || '#'} target="_blank" rel="noreferrer" className="block px-3 py-2 hover:bg-bg/40">
      <div className="flex items-center gap-2 text-[11px] mb-1">
        ...
        {article.ticker && <span className="font-mono text-accent">${article.ticker}</span>}
