...
export function TopBar() {
  ...
  const doFetch = async () => {
    ...
    try {
      ...
      toast(
        `${data.quotes_updated ?? 0} quotes; +${data.new_articles ?? 0} new articles${data.updated_articles !== undefined ? `, ${data.updated_articles} refreshed` : ''}; +${socialNew} social${socialUpdated ? `, ${socialUpdated} refreshed` : ''}`,
        undefined,
    ...
    if (watching) {
      ...
      es.addEventListener('line', (e) => {
        ...
        if (d.new !== undefined) {
          ...
          toast(
            `${d.quotes_updated ?? 0} quotes; +${d.new} new articles${d.updated > 0 ? `, ${d.updated} refreshed` : ''}; +${d.social_new ?? 0} social${d.social_updated > 0 ? `, ${d.social_updated} refreshed` : ''}`,
            undefined,
  ...
  }, [watching, watchInterval, mutateStatus, revalidateDashboardData])
      ...
      <header className="bg-surface border-b border-border flex-shrink-0">
        <div className="min-h-14 flex items-center gap-3 px-4 py-2">
          ...
          <button
            ...
            title={cooldownRemaining > 0 ? `Fetch available in ${cooldownRemaining}s` : 'Fetch latest news'}
          ...
          <div className="hidden sm:flex items-center gap-2">
            {status && <StatusBadge ok={status.ok} label={`${status.database?.articles ?? 0} articles`} />}
